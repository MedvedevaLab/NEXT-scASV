...
def process_variants(variants, samples, samples_genotype_id, sample_filepaths):
    # Create TabixFile objects
    sample_infiles = {sample: pysam.TabixFile(filepath) for sample, filepath in sample_filepaths.items()}
    ...
    try:
        for var in variants:
            ...
            for sample in samples:
                ...
                except (ValueError, GenotypeError, StopIteration) as e:
                    if isinstance(e, ValueError):
                        logging.critical(e)
                        logging.critical(sample)
                        sys.exit(1)
                    elif isinstance(e, GenotypeError):
            ...
            if n_total_ref + n_total_alt > 0:
                results.append((var, samples_format))
    finally:
        # Clean up TabixFile objects
        for filehandle in sample_infiles.values():
            filehandle.close()
...
def main(argv = sys.argv[1:]):
    ...
    with open(args.sample_map_file) as f:
        for line in f:
            ...
            sample_filepaths[sample] = het_counts_filepath
    ...
    # Process variants
    variants = infile.fetch(contig=args.chrom)
    results = process_variants(variants, samples, samples_genotype_id, sample_filepaths)

    # Write results to output file
    for var, samples_format in results:
        outvar = outfile_per_sample.new_record(
            contig=var.contig,
            start=var.start,
            stop=var.stop,
            alleles=var.alleles,
